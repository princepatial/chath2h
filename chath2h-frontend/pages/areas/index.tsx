...
const AreasPage = (_props: InferGetServerSidePropsType<typeof getServerSideProps>) => {
  ...
  const sortedCategories = useMemo(() => {
    if (!Array.isArray(currentAreasList) || currentAreasList.length < 1) return [];
    ...
    if (indexItem > -1) {
      const splicedList = newList.splice(indexItem, 1); // Correct splice to remove 1 item
      return [...newList, splicedList[0]];
    }
